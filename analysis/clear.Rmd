...
The math behind setting up the model with conjugate priors in markdown form is as follows:
...
We will look at each Secondary outcome separately here for simplicity, so 
...
Intuitively, one may have more negative outcomes if they have a stronger primary response. We cannot calculate this without the original data but here assume a correlation of \rho=0.5. We can adaptc
...
library(ggplot2)
library(kernlab)  # For kernel density estimation
...
Now we might liek to see what is the odds of not having a clinically meaningful primary outcome and having a true logOR of the secondary outcome greater than 0.  In our model, we are assuming a correlation between the primary and secondary outcomes, specified by \rho = 0.4.This correlation is factored into the covariance matrix of the bivariate normal distribution from which we sample the posterior distributions of the primary and secondary outcomes.
